...
def main() -> int:
    ...
    completed_banks = (
        (23, '.DSB $4000 - $1F15, $00', 'Bank23End:', 'Bank 23'),
        (29, '.DSB $4000 - $287B, $00', 'Bank29End:', 'Bank 29'),
    )
    for bank, padding, end_label, description in completed_banks:
        source = banks[bank].read_text(errors='replace')
        raw_lines = [line for line in source.splitlines() if line.lower().startswith('.db ')]
        if raw_lines:
            print(f'FAIL {description} contains generated byte lines after being marked source-complete')
            return 1
        if padding not in source or end_label not in source:
            print(f'FAIL {description} lost its exact padding or bank-end boundary')
            return 1
        print(f'OK {description} is source-complete with exact named end padding')
