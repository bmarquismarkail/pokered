_Route10SuperNerd1BattleText:
WLA_GLOBAL_Route10SuperNerd1BattleText:
	text "Wow, are you a"
	line "#MANIAC too?"
	cont "Want to see my"
	cont "collection?"
	done

_Route10SuperNerd1EndBattleText:
WLA_GLOBAL_Route10SuperNerd1EndBattleText:
	text "Humph."
	line "I'm not angry!"
	prompt

_Route10SuperNerd1AfterBattleText:
WLA_GLOBAL_Route10SuperNerd1AfterBattleText:
	text "I have more rare"
	line "#MON at home!"
	done

_Route10Hiker1BattleText:
WLA_GLOBAL_Route10Hiker1BattleText:
	text "Ha-hahah-ah-ha!"
	done

_Route10Hiker1EndBattleText:
WLA_GLOBAL_Route10Hiker1EndBattleText:
	text "Ha-haha!"
	line "Not laughing!"
	cont "Ha-hay fever!"
	cont "Haha-ha-choo!"
	prompt

_Route10Hiker1AfterBattleText:
WLA_GLOBAL_Route10Hiker1AfterBattleText:
	text "Haha-ha-choo!"
	line "Ha-choo!"
	cont "Snort! Snivel!"
	done

_Route10SuperNerd2BattleText:
WLA_GLOBAL_Route10SuperNerd2BattleText:
	text "Hi kid, want to"
	line "see my #MON?"
	done

_Route10SuperNerd2EndBattleText:
WLA_GLOBAL_Route10SuperNerd2EndBattleText:
	text "Oh no!"
	line "My #MON!"
	prompt

_Route10SuperNerd2AfterBattleText:
WLA_GLOBAL_Route10SuperNerd2AfterBattleText:
	text "I don't like you"
	line "for beating me!"
	done

_Route10CooltrainerF1BattleText:
WLA_GLOBAL_Route10CooltrainerF1BattleText:
	text "I've been to a"
	line "#MON GYM a few"
	cont "times. But, I"
	cont "lost each time."
	done

_Route10CooltrainerF1EndBattleText:
WLA_GLOBAL_Route10CooltrainerF1EndBattleText:
	text "Ohh!"
	line "Blew it again!"
	prompt

_Route10CooltrainerF1AfterBattleText:
WLA_GLOBAL_Route10CooltrainerF1AfterBattleText:
	text "I noticed some"
	line "#MANIACs"
	cont "prowling around."
	done

_Route10Hiker2BattleText:
WLA_GLOBAL_Route10Hiker2BattleText:
	text "Ah! This mountain"
	line "air is delicious!"
	done

_Route10Hiker2EndBattleText:
WLA_GLOBAL_Route10Hiker2EndBattleText:
	text "That"
	line "cleared my head!"
	prompt

_Route10Hiker2AfterBattleText:
WLA_GLOBAL_Route10Hiker2AfterBattleText:
	text "I feel bloated on"
	line "mountain air!"
	done

_Route10CooltrainerF2BattleText:
WLA_GLOBAL_Route10CooltrainerF2BattleText:
	text "I'm feeling a bit"
	line "faint from this"
	cont "tough hike."
	done

_Route10CooltrainerF2EndBattleText:
WLA_GLOBAL_Route10CooltrainerF2EndBattleText:
	text "I'm"
	line "not up to it!"
	prompt

_Route10CooltrainerF2AfterBattleText:
WLA_GLOBAL_Route10CooltrainerF2AfterBattleText:
	text "The #MON here"
	line "are so chunky!"
	cont "There should be a"
	cont "pink one with a"
	cont "floral pattern!"
	done

_Route10RockTunnelSignText:
WLA_GLOBAL_Route10RockTunnelSignText:
	text "ROCK TUNNEL"
	done

_Route10PowerPlantSignText:
WLA_GLOBAL_Route10PowerPlantSignText:
	text "POWER PLANT"
	done
