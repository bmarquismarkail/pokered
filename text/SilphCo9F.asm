SilphCo9FNurseYouLookTiredText:
	text "You look tired!"
	line "You should take a"
	cont "quick nap!"
	prompt

SilphCo9FNurseDontGiveUpText:
	text "Don't give up!"
	done

SilphCo9FNurseThankYouText:
	text "Thank you so"
	line "much!"
	done

_SilphCo9FRocket1BattleText:
WLA_GLOBAL_SilphCo9FRocket1BattleText:
	text "Your #MON seem"
	line "to adore you, kid!"
	done

_SilphCo9FRocket1EndBattleText:
WLA_GLOBAL_SilphCo9FRocket1EndBattleText:
	text "Ghaaah!"
	prompt

_SilphCo9FRocket1AfterBattleText:
WLA_GLOBAL_SilphCo9FRocket1AfterBattleText:
	text "If I had started"
	line "as a trainer at"
	cont "your age..."
	done

_SilphCo9FScientistBattleText:
WLA_GLOBAL_SilphCo9FScientistBattleText:
	text "Your #MON have"
	line "weak points! I"
	cont "can nail them!"
	done

_SilphCo9FScientistEndBattleText:
WLA_GLOBAL_SilphCo9FScientistEndBattleText:
	text "You"
	line "hammered me!"
	prompt

_SilphCo9FScientistAfterBattleText:
WLA_GLOBAL_SilphCo9FScientistAfterBattleText:
	text "Exploiting weak"
	line "spots does work!"
	cont "Think about"
	cont "element types!"
	done

_SilphCo9FRocket2BattleText:
WLA_GLOBAL_SilphCo9FRocket2BattleText:
	text "I am one of the 4"
	line "ROCKET BROTHERS!"
	done

_SilphCo9FRocket2EndBattleText:
WLA_GLOBAL_SilphCo9FRocket2EndBattleText:
	text "Warg!"
	line "Brothers, I lost!"
	prompt

_SilphCo9FRocket2AfterBattleText:
WLA_GLOBAL_SilphCo9FRocket2AfterBattleText:
	text "My brothers will"
	line "avenge me!"
	done
