_Route16Biker1BattleText:
WLA_GLOBAL_Route16Biker1BattleText:
	text "What do you want?"
	done

_Route16Biker1EndBattleText:
WLA_GLOBAL_Route16Biker1EndBattleText:
	text "Don't you"
	line "dare laugh!"
	prompt

_Route16Biker1AfterBattleText:
WLA_GLOBAL_Route16Biker1AfterBattleText:
	text "We like just"
	line "hanging here,"
	cont "what's it to you?"
	done

_Route16Biker2BattleText:
WLA_GLOBAL_Route16Biker2BattleText:
	text "Nice BIKE!"
	line "Hand it over!"
	done

_Route16Biker2EndBattleText:
WLA_GLOBAL_Route16Biker2EndBattleText:
	text "Knock"
	line "out!"
	prompt

_Route16Biker2AfterBattleText:
WLA_GLOBAL_Route16Biker2AfterBattleText:
	text "Forget it, who"
	line "needs your BIKE!"
	done

_Route16Biker3BattleText:
WLA_GLOBAL_Route16Biker3BattleText:
	text "Come out and play,"
	line "little mouse!"
	done

_Route16Biker3EndBattleText:
WLA_GLOBAL_Route16Biker3EndBattleText:
	text "You"
	line "little rat!"
	prompt

_Route16Biker3AfterBattleText:
WLA_GLOBAL_Route16Biker3AfterBattleText:
	text "I hate losing!"
	line "Get away from me!"
	done

_Route16biker4BattleText:
WLA_GLOBAL_Route16biker4BattleText:
	text "Hey, you just"
	line "bumped me!"
	done

_Route16Biker4EndBattleText:
WLA_GLOBAL_Route16Biker4EndBattleText:
	text "Kaboom!"
	prompt

_Route16Biker4AfterBattleText:
WLA_GLOBAL_Route16Biker4AfterBattleText:
	text "You can also get"
	line "to FUCHSIA from"
	cont "VERMILION using a"
	cont "coastal road."
	done

_Route16Biker5BattleText:
WLA_GLOBAL_Route16Biker5BattleText:
	text "I'm feeling"
	line "hungry and mean!"
	done

_Route16Biker5EndBattleText:
WLA_GLOBAL_Route16Biker5EndBattleText:
	text "Bad,"
	line "bad, bad!"
	prompt

_Route16Biker5AfterBattleText:
WLA_GLOBAL_Route16Biker5AfterBattleText:
	text "I like my #MON"
	line "ferocious! They"
	cont "tear up enemies!"
	done

_Route16Biker6BattleText:
WLA_GLOBAL_Route16Biker6BattleText:
	text "Sure, I'll go!"
	done

_Route16Biker6EndBattleText:
WLA_GLOBAL_Route16Biker6EndBattleText:
	text "Don't make"
	line "me mad!"
	prompt

_Route16Biker6AfterBattleText:
WLA_GLOBAL_Route16Biker6AfterBattleText:
	text "I like harassing"
	line "people with my"
	cont "vicious #MON!"
	done

_Route16Text7:
WLA_GLOBAL_Route16Text7:
	text "A sleeping #MON"
	line "blocks the way!"
	done

_Route16SnorlaxWokeUpText:
WLA_GLOBAL_Route16SnorlaxWokeUpText:
	text "SNORLAX woke up!"

	para "It attacked in a"
	line "grumpy rage!"
	done

_Route16SnorlaxReturnedToMountainsText:
WLA_GLOBAL_Route16SnorlaxReturnedToMountainsText:
	text "With a big yawn,"
	line "SNORLAX returned"
	cont "to the mountains!"
	done

_Route16CyclingRoadSignText:
WLA_GLOBAL_Route16CyclingRoadSignText:
	text "Enjoy the slope!"
	line "CYCLING ROAD"
	done

_Route16SignText:
WLA_GLOBAL_Route16SignText:
	text "ROUTE 16"
	line "CELADON CITY -"
	cont "FUCHSIA CITY"
	done
