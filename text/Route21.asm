_Route21Fisher1BattleText:
WLA_GLOBAL_Route21Fisher1BattleText:
	text "You want to know"
	line "if the fish are"
	cont "biting?"
	done

_Route21Fisher1EndBattleText:
WLA_GLOBAL_Route21Fisher1EndBattleText:
	text "Dang!"
	prompt

_Route21Fisher1AfterBattleText:
WLA_GLOBAL_Route21Fisher1AfterBattleText:
	text "I can't catch"
	line "anything good!"
	done

_Route21Fisher2BattleText:
WLA_GLOBAL_Route21Fisher2BattleText:
	text "I got a big haul!"
	line "Wanna go for it?"
	done

_Route21Fisher2EndBattleText:
WLA_GLOBAL_Route21Fisher2EndBattleText:
	text "Darn"
	line "MAGIKARP!"
	prompt

_Route21Fisher2AfterBattleText:
WLA_GLOBAL_Route21Fisher2AfterBattleText:
	text "I seem to only"
	line "catch MAGIKARP!"
	done

_Route21Swimmer1BattleText:
WLA_GLOBAL_Route21Swimmer1BattleText:
	text "The sea cleanses"
	line "my body and soul!"
	done

_Route21Swimmer1EndBattleText:
WLA_GLOBAL_Route21Swimmer1EndBattleText:
	text "Ayah!"
	prompt

_Route21Swimmer1AfterBattleText:
WLA_GLOBAL_Route21Swimmer1AfterBattleText:
	text "I like the"
	line "mountains too!"
	done

_Route21Swimmer2BattleText:
WLA_GLOBAL_Route21Swimmer2BattleText:
	text "What's wrong with"
	line "me swimming?"
	done

_Route21Swimmer2EndBattleText:
WLA_GLOBAL_Route21Swimmer2EndBattleText:
	text "Cheap"
	line "shot!"
	prompt

_Route21Swimmer2AfterBattleText:
WLA_GLOBAL_Route21Swimmer2AfterBattleText:
	text "I look like what?"
	line "A studded inner"
	cont "tube? Get lost!"
	done

_Route21Swimmer3BattleText:
WLA_GLOBAL_Route21Swimmer3BattleText:
	text "I caught all my"
	line "#MON at sea!"
	done

_Route21Swimmer3EndBattleText:
WLA_GLOBAL_Route21Swimmer3EndBattleText:
	text "Diver!!"
	line "Down!!"
	prompt

_Route21Swimmer3AfterBattleText:
WLA_GLOBAL_Route21Swimmer3AfterBattleText:
	text "Where'd you catch"
	line "your #MON?"
	done

_Route21Swimmer4BattleText:
WLA_GLOBAL_Route21Swimmer4BattleText:
	text "Right now, I'm in"
	line "a triathlon meet!"
	done

_Route21Swimmer4EndBattleText:
WLA_GLOBAL_Route21Swimmer4EndBattleText:
	text "Pant..."
	line "pant...pant..."
	prompt

_Route21Swimmer4AfterBattleText:
WLA_GLOBAL_Route21Swimmer4AfterBattleText:
	text "I'm beat!"
	line "But, I still have"
	cont "the bike race and"
	cont "marathon left!"
	done

_Route21Swimmer5BattleText:
WLA_GLOBAL_Route21Swimmer5BattleText:
	text "Ahh! Feel the sun"
	line "and the wind!"
	done

_Route21Swimmer5EndBattleText:
WLA_GLOBAL_Route21Swimmer5EndBattleText:
	text "Yow!"
	line "I lost!"
	prompt

_Route21Swimmer5AfterBattleText:
WLA_GLOBAL_Route21Swimmer5AfterBattleText:
	text "I'm sunburnt to a"
	line "crisp!"
	done

_Route21Fisher3BattleText:
WLA_GLOBAL_Route21Fisher3BattleText:
	text "Hey, don't scare"
	line "away the fish!"
	done

_Route21Fisher3EndBattleText:
WLA_GLOBAL_Route21Fisher3EndBattleText:
	text "Sorry!"
	line "I didn't mean it!"
	prompt

_Route21Fisher3AfterBattleText:
WLA_GLOBAL_Route21Fisher3AfterBattleText:
	text "I was just angry"
	line "that I couldn't"
	cont "catch anything."
	done

_Route21Fisher4BattleText:
WLA_GLOBAL_Route21Fisher4BattleText:
	text "Keep me company"
	line "'til I get a hit!"
	done

_Route21Fisher4EndBattleText:
WLA_GLOBAL_Route21Fisher4EndBattleText:
	text "That"
	line "burned some time."
	prompt

_Route21Fisher4AfterBattleText:
WLA_GLOBAL_Route21Fisher4AfterBattleText:
	text "Oh wait! I got a"
	line "bite! Yeah!"
	done
