_RockTunnelB1FCooltrainerF1BattleText:
WLA_GLOBAL_RockTunnelB1FCooltrainerF1BattleText:
	text "Hikers leave twigs"
	line "as trail markers."
	done

_RockTunnelB1FCooltrainerF1EndBattleText:
WLA_GLOBAL_RockTunnelB1FCooltrainerF1EndBattleText:
	text "Ohhh!"
	line "I did my best!"
	prompt

_RockTunnelB1FCooltrainerF1AfterBattleText:
WLA_GLOBAL_RockTunnelB1FCooltrainerF1AfterBattleText:
	text "I want to go "
	line "home!"
	done

_RockTunnelB1FHiker1BattleText:
WLA_GLOBAL_RockTunnelB1FHiker1BattleText:
	text "Hahaha! Can you"
	line "beat my power?"
	done

_RockTunnelB1FHiker1EndBattleText:
WLA_GLOBAL_RockTunnelB1FHiker1EndBattleText:
	text "Oops!"
	line "Out-muscled!"
	prompt

_RockTunnelB1FHiker1AfterBattleText:
WLA_GLOBAL_RockTunnelB1FHiker1AfterBattleText:
	text "I go for power"
	line "because I hate"
	cont "thinking!"
	done

_RockTunnelB1FSuperNerd1BattleText:
WLA_GLOBAL_RockTunnelB1FSuperNerd1BattleText:
	text "You have a"
	line "#DEX?"
	cont "I want one too!"
	done

_RockTunnelB1FSuperNerd1EndBattleText:
WLA_GLOBAL_RockTunnelB1FSuperNerd1EndBattleText:
	text "Shoot!"
	line "I'm so jealous!"
	prompt

_RockTunnelB1FSuperNerd1AfterBattleText:
WLA_GLOBAL_RockTunnelB1FSuperNerd1AfterBattleText:
	text "When you finish"
	line "your #DEX, can"
	cont "I have it?"
	done

_RockTunnelB1FSuperNerd2BattleText:
WLA_GLOBAL_RockTunnelB1FSuperNerd2BattleText:
	text "Do you know about"
	line "costume players?"
	done

_RockTunnelB1FSuperNerd2EndBattleText:
WLA_GLOBAL_RockTunnelB1FSuperNerd2EndBattleText:
	text "Well,"
	line "that's that."
	prompt

_RockTunnelB1FSuperNerd2AfterBattleText:
WLA_GLOBAL_RockTunnelB1FSuperNerd2AfterBattleText:
	text "Costume players"
	line "dress up as"
	cont "#MON for fun."
	done

_RockTunnelB1FHiker2BattleText:
WLA_GLOBAL_RockTunnelB1FHiker2BattleText:
	text "My #MON"
	line "techniques will"
	cont "leave you crying!"
	done

_RockTunnelB1FHiker2EndBattleText:
WLA_GLOBAL_RockTunnelB1FHiker2EndBattleText:
	text "I give!"
	line "You're a better"
	cont "technician!"
	prompt

_RockTunnelB1FHiker2AfterBattleText:
WLA_GLOBAL_RockTunnelB1FHiker2AfterBattleText:
	text "In mountains,"
	line "you'll often find"
	cont "rock-type #MON."
	done

_RockTunnelB1FCooltrainerF2BattleText:
WLA_GLOBAL_RockTunnelB1FCooltrainerF2BattleText:
	text "I don't often"
	line "come here, but I"
	cont "will fight you."
	done

_RockTunnelB1FCooltrainerF2EndBattleText:
WLA_GLOBAL_RockTunnelB1FCooltrainerF2EndBattleText:
	text "Oh!"
	line "I lost!"
	prompt

_RockTunnelB1FCooltrainerF2AfterBattleText:
WLA_GLOBAL_RockTunnelB1FCooltrainerF2AfterBattleText:
	text "I like tiny"
	line "#MON, big ones"
	cont "are too scary!"
	done

_RockTunnelB1FHiker3BattleText:
WLA_GLOBAL_RockTunnelB1FHiker3BattleText:
	text "Hit me with your"
	line "best shot!"
	done

_RockTunnelB1FHiker3EndBattleText:
WLA_GLOBAL_RockTunnelB1FHiker3EndBattleText:
	text "Fired"
	line "away!"
	prompt
