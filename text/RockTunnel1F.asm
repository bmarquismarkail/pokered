_RockTunnel1FHiker1BattleText:
WLA_GLOBAL_RockTunnel1FHiker1BattleText:
	text "This tunnel goes"
	line "a long way, kid!"
	done

_RockTunnel1FHiker1EndBattleText:
WLA_GLOBAL_RockTunnel1FHiker1EndBattleText:
	text "Doh!"
	line "You win!"
	prompt

_RockTunnel1FHiker1AfterBattleText:
WLA_GLOBAL_RockTunnel1FHiker1AfterBattleText:
	text "Watch for ONIX!"
	line "It can put the"
	cont "squeeze on you!"
	done

_RockTunnel1FHiker2BattleText:
WLA_GLOBAL_RockTunnel1FHiker2BattleText:
	text "Hmm. Maybe I'm"
	line "lost in here..."
	done

_RockTunnel1FHiker2EndBattleText:
WLA_GLOBAL_RockTunnel1FHiker2EndBattleText:
	text "Ease up!"
	line "What am I doing?"
	cont "Which way is out?"
	prompt

_RockTunnel1FHiker2AfterBattleText:
WLA_GLOBAL_RockTunnel1FHiker2AfterBattleText:
	text "That sleeping"
	line "#MON on ROUTE"
	cont "12 forced me to"
	cont "take this detour."
	done

_RockTunnel1FHiker3BattleText:
WLA_GLOBAL_RockTunnel1FHiker3BattleText:
	text "Outsiders like"
	line "you need to show"
	cont "me some respect!"
	done

_RockTunnel1FHiker3EndBattleText:
WLA_GLOBAL_RockTunnel1FHiker3EndBattleText:
	text "I give!"
	prompt

_RockTunnel1FHiker3AfterBattleText:
WLA_GLOBAL_RockTunnel1FHiker3AfterBattleText:
	text "You're talented"
	line "enough to hike!"
	done

_RockTunnel1FSuperNerdBattleText:
WLA_GLOBAL_RockTunnel1FSuperNerdBattleText:
	text "#MON fight!"
	line "Ready, go!"
	done

_RockTunnel1FSuperNerdEndBattleText:
WLA_GLOBAL_RockTunnel1FSuperNerdEndBattleText:
	text "Game"
	line "over!"
	prompt

_RockTunnel1FSuperNerdAfterBattleText:
WLA_GLOBAL_RockTunnel1FSuperNerdAfterBattleText:
	text "Oh well, I'll get"
	line "a ZUBAT as I go!"
	done

_RockTunnel1FCooltrainerF1BattleText:
WLA_GLOBAL_RockTunnel1FCooltrainerF1BattleText:
	text "Eek! Don't try"
	line "anything funny in"
	cont "the dark!"
	done

_RockTunnel1FCooltrainerF1EndBattleText:
WLA_GLOBAL_RockTunnel1FCooltrainerF1EndBattleText:
	text "It"
	line "was too dark!"
	prompt

_RockTunnel1FCooltrainerF1AfterBattleText:
WLA_GLOBAL_RockTunnel1FCooltrainerF1AfterBattleText:
	text "I saw a MACHOP"
	line "in this tunnel!"
	done

_RockTunnel1FCooltrainerF2BattleText:
WLA_GLOBAL_RockTunnel1FCooltrainerF2BattleText:
	text "I came this far"
	line "for #MON!"
	done

_RockTunnel1FCooltrainerF2EndBattleText:
WLA_GLOBAL_RockTunnel1FCooltrainerF2EndBattleText:
	text "I'm"
	line "out of #MON!"
	prompt

_RockTunnel1FCooltrainerF2AfterBattleText:
WLA_GLOBAL_RockTunnel1FCooltrainerF2AfterBattleText:
	text "You looked cute"
	line "and harmless!"
	done

_RockTunnel1FCooltrainerF3BattleText:
WLA_GLOBAL_RockTunnel1FCooltrainerF3BattleText:
	text "You have #MON!"
	line "Let's start!"
	done

_RockTunnel1FCooltrainerF3EndBattleText:
WLA_GLOBAL_RockTunnel1FCooltrainerF3EndBattleText:
	text "You"
	line "play hard!"
	prompt

_RockTunnel1FCooltrainerF3AfterBattleText:
WLA_GLOBAL_RockTunnel1FCooltrainerF3AfterBattleText:
	text "Whew! I'm all"
	line "sweaty now!"
	done

_RockTunnel1FSignText:
WLA_GLOBAL_RockTunnel1FSignText:
	text "ROCK TUNNEL"
	line "CERULEAN CITY -"
	cont "LAVENDER TOWN"
	done
