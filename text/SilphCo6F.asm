_SilphCo6FSilphWorkerM1TookOverTheBuildingText:
WLA_GLOBAL_SilphCo6FSilphWorkerM1TookOverTheBuildingText:
	text "The ROCKETs came"
	line "and took over the"
	cont "building!"
	done

_SilphCo6FSilphWorkerM1BackToWorkText:
WLA_GLOBAL_SilphCo6FSilphWorkerM1BackToWorkText:
	text "Well, better get"
	line "back to work!"
	done

_SilphCo6FSilphWorkerMHelpMePleaseText:
WLA_GLOBAL_SilphCo6FSilphWorkerMHelpMePleaseText:
	text "Oh dear, oh dear."
	line "Help me please!"
	done

_SilphCo6FSilphWorkerMWeGotEngagedText:
WLA_GLOBAL_SilphCo6FSilphWorkerMWeGotEngagedText:
	text "We got engaged!"
	line "Heheh!"
	done

_SilphCo6FSilphWorkerF1SuchACowardText:
WLA_GLOBAL_SilphCo6FSilphWorkerF1SuchACowardText:
	text "Look at him! He's"
	line "such a coward!"
	done

_SilphCo6FSilphWorkerF1HaveToMarryHimText:
WLA_GLOBAL_SilphCo6FSilphWorkerF1HaveToMarryHimText:
	text "I feel so sorry"
	line "for him, I have"
	cont "to marry him!"
	done

_SilphCo6FSilphWorkerF2TeamRocketConquerWorldText:
WLA_GLOBAL_SilphCo6FSilphWorkerF2TeamRocketConquerWorldText:
	text "TEAM ROCKET is"
	line "trying to conquer"
	cont "the world with"
	cont "#MON!"
	done

_SilphCo6FSilphWorkerF2TeamRocketRanText:
WLA_GLOBAL_SilphCo6FSilphWorkerF2TeamRocketRanText:
	text "TEAM ROCKET ran"
	line "because of you!"
	done

_SilphCo6FSilphWorkerM3TargetedSilphText:
WLA_GLOBAL_SilphCo6FSilphWorkerM3TargetedSilphText:
	text "They must have"
	line "targeted SILPH"
	cont "for our #MON"
	cont "products."
	done

_SilphCo6FSilphWorkerM3WorkForSilphText:
WLA_GLOBAL_SilphCo6FSilphWorkerM3WorkForSilphText:
	text "Come work for"
	line "SILPH when you"
	cont "get older!"
	done

_SilphCo6FRocket1BattleText:
WLA_GLOBAL_SilphCo6FRocket1BattleText:
	text "I am one of the 4"
	line "ROCKET BROTHERS!"
	done

_SilphCo6FRocket1EndBattleText:
WLA_GLOBAL_SilphCo6FRocket1EndBattleText:
	text "Flame"
	line "out!"
	prompt

_SilphCo6FRocket1AfterBattleText:
WLA_GLOBAL_SilphCo6FRocket1AfterBattleText:
	text "No matter!"
	line "My brothers will"
	cont "avenge me!"
	done

_SilphCo6FScientistBattleText:
WLA_GLOBAL_SilphCo6FScientistBattleText:
	text "That rotten"
	line "PRESIDENT!"

	para "He shouldn't have"
	line "sent me to the"
	cont "TIKSI BRANCH!"
	done

_SilphCo6FScientistEndBattleText:
WLA_GLOBAL_SilphCo6FScientistEndBattleText:
	text "Shoot!"
	prompt

_SilphCo6FScientistAfterBattleText:
WLA_GLOBAL_SilphCo6FScientistAfterBattleText:
	text "TIKSI BRANCH?"
	line "It's in Russian"
	cont "no man's land!"
	done

_SilphCo6FRocket2BattleText:
WLA_GLOBAL_SilphCo6FRocket2BattleText:
	text "You dare betray"
	line "TEAM ROCKET?"
	done

_SilphCo6FRocket2EndBattleText:
WLA_GLOBAL_SilphCo6FRocket2EndBattleText:
	text "You"
	line "traitor!"
	prompt

_SilphCo6FRocket2AfterBattleText:
WLA_GLOBAL_SilphCo6FRocket2AfterBattleText:
	text "If you stand for"
	line "justice, you"
	cont "betray evil!"
	done
