_RocketHideoutB1FRocket5EndBattleText:
WLA_GLOBAL_RocketHideoutB1FRocket5EndBattleText:
	text "Why...?@"
	text_end

_RocketHideoutB1FRocket1BattleText:
WLA_GLOBAL_RocketHideoutB1FRocket1BattleText:
	text "Who are you? How"
	line "did you get here?"
	done

_RocketHideoutB1FRocket1EndBattleText:
WLA_GLOBAL_RocketHideoutB1FRocket1EndBattleText:
	text "Oww!"
	line "Beaten!"
	prompt

_RocketHideoutB1FRocket1AfterBattleText:
WLA_GLOBAL_RocketHideoutB1FRocket1AfterBattleText:
	text "Are you dissing"
	line "TEAM ROCKET?"
	done

_RocketHideoutB1FRocket2BattleText:
WLA_GLOBAL_RocketHideoutB1FRocket2BattleText:
	text "You broke into"
	line "our operation?"
	done

_RocketHideoutB1FRocket2EndBattleText:
WLA_GLOBAL_RocketHideoutB1FRocket2EndBattleText:
	text "Burnt!"
	prompt

_RocketHideoutB1FRocket2AfterBattleText:
WLA_GLOBAL_RocketHideoutB1FRocket2AfterBattleText:
	text "You're not going"
	line "to get away with"
	cont "this, brat!"
	done

_RocketHideoutB1FRocket3BattleText:
WLA_GLOBAL_RocketHideoutB1FRocket3BattleText:
	text "Intruder alert!"
	done

_RocketHideoutB1FRocket3EndBattleText:
WLA_GLOBAL_RocketHideoutB1FRocket3EndBattleText:
	text "I"
	line "can't do it!"
	prompt

_RocketHideoutB1FRocket3AfterBattleText:
WLA_GLOBAL_RocketHideoutB1FRocket3AfterBattleText:
	text "SILPH SCOPE?"
	line "I don't know"
	cont "where it is!"
	done

_RocketHideoutB1FRocket4BattleText:
WLA_GLOBAL_RocketHideoutB1FRocket4BattleText:
	text "Why did you come"
	line "here?"
	done

_RocketHideoutB1FRocket4EndBattleText:
WLA_GLOBAL_RocketHideoutB1FRocket4EndBattleText:
	text "This"
	line "won't do!"
	prompt

_RocketHideoutB1FRocket4AfterBattleText:
WLA_GLOBAL_RocketHideoutB1FRocket4AfterBattleText:
	text "OK, I'll talk!"
	line "Take the elevator"
	cont "to see my BOSS!"
	done

_RocketHideoutB1FRocket5BattleText:
WLA_GLOBAL_RocketHideoutB1FRocket5BattleText:
	text "Are you lost, you"
	line "little rat?"
	done

_RocketHideoutB1FRocket5AfterBattleText:
WLA_GLOBAL_RocketHideoutB1FRocket5AfterBattleText:
	text "Uh-oh, that fight"
	line "opened the door!"
	done
